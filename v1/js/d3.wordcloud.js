    ...
    wordcloud.element = function(x) {
      ...
      wordcloud.selector = function(x) {
        ...
        wordcloud.transitionDuration = function(x) {
          ...
          */
          $.ajax({
           url: '../php/wordClicked.php',
           // type: 'default GET (Other values: POST)',
           // dataType: 'default: Intelligent Guess (Other values: xml, json, script, or html)',
           data: {clicked_word: d.text,
            action: "wordClicked"},
          })
          .done(function(result) {
            console.log("this is the reulst to store");
            // console.log(result);
            // console.log(JSON.parse(result));
            console.log("success");
            localStorage.setItem("clicked_word", d.text);
            localStorage.setItem("paper_to_show", result);
            setTimeout(function(){window.location = "paperList.html";}, 2000);
          })
          .fail(function() {
           console.log("error");
         })
